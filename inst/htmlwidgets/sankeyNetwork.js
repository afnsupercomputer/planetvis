HTMLWidgets.widget({
  ...
  renderValue: function (el, x, instance) {
    ...
    var trade_info_nodes = x.options.trade_info_nodes;
    ...
    var node = svg
      ...
      .on("mouseover", function (d) {
        ...
        Shiny.setInputValue("sankey-node_piechart", d.name, { priority: "event" });
      })
      .on("mouseout", function (d) {
        link
          .filter(function (d1, i) {
      ...
      .on("mousedown.drag", Shiny.onInputChange("sankey-node_zoom", null))
      .call(function () {
        manualLayout();
      });
    ...
    function format_value(value, n_decimals){
      if (n_decimals !== null){
        return (Math.round(value * 100) / 100).toFixed(n_decimals);
      }
      else{
        return(format(value));
      }
    ...
    node
      ...
      .html(function (d) {
        return (
          ...
          ": " +
          "<br>" +
          format_value(d.value, options.n_decimals) +
          " " +
          options.units +
          "</pre>"
        );
    ...
    d3.selectAll('.node rect')
      .on("click", function (d) {
        if (node_to_zoom.includes(d.name) & d.name != 'Milk') {
          ...
          Shiny.setInputValue("sankey-trade_info", null);
          Shiny.setInputValue("sankey-node_zoom", d.name, { priority: "event" });
    ...
    if (options.units == 'kt' & options.data_source != 'Demo Resources/Outcomes') {
      ...
      d3.selectAll(".info-circle-trade")
        .on("click", function (d) { Shiny.setInputValue("sankey-trade_info", { click: 1, node_name: d.name }, { priority: "event" }); });
    }
    ...
    // Add inform button for pulp
     const pulp = d3.selectAll('.node text')
      ...
      d3.selectAll(".info-circle-pulp")
        .on("click", function (d) { Shiny.setInputValue("sankey-pulp_info", { click: 1, node_name: d.name }, { priority: "event" }); });
    ...
    else if (options.units == 'tons of protein')
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " tons (" + "t" + ") protein pa";
    }
    else if (options.units == 'tons of adj. protein')
    {

      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " tons (" + "t" + ") adj. protein pa";
    }
    else if (options.units == 'tons of fat')
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " tons (" + "t" + ") fat pa";
    }
    else if (options.units == 'billion kcal')
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " billion kilocalories (" + "kcal" + ") energy pa";
    }
    else if (options.units == "mass gram/capita/day")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " mass in grams pc pd";
    }
    else if (options.units == "mass kg/capita/year")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " mass in kg pc pa";
    }
    else if (options.units == "adj. protein gram/capita/day")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " adj. protein in grams pc pd";
    }
    else if (options.units == "adj. protein gram/capita/year")
    {
       scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " adj. protein in grams pc pa";
    }
    else if (options.units == "kcal/capita/day")
    {
       scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " energy in kcals pc pd";
    }
    else if (options.units == "kcal/capita/year")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " energy in kcals pc pa";
    }
    else if (options.units == "protein gram/capita/year")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " protein in grams pc pa";
    }
    else if (options.units == "protein gram/capita/day")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " protein in grams pc pd";
    }
    else if (options.units == "fat gram/capita/day")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " fat in grams pc pd";

    }
    else if (options.units == "fat kg/capita/year")
    {
      scale_text.textContent =
         "25 px ~  " + "\n" + format(scale_value.toFixed(0)) + " fat in kg pc pa";
    }
    ...
    if (options.region == "World" & !node_to_zoom.includes("Recycle")) {
   ...
   d3.selectAll(".info-top-producers")
      .on("click", function (d) { Shiny.setInputValue("sankey-top_producers",
      { click: 1, node_name: d.name }, { priority: "event" }); });
    ...
    x_coord = [...new Set(x_coord)].sort(function(a, b) {
                return a - b;});
